...
| CRM companies | PARTIAL | Canonical companies-only CRM workspace | Workspace records plus company-intelligence enrichment and immutable per-attempt research snapshots | Record events, research worker, autonomous `crm.company.enrich` command | `crm.read/manage` | Current projection and provenance history are durable; provider coverage and a dedicated company schema remain future work |
