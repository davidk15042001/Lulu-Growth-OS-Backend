...
| Storefront customer payments | EXISTS | Published storefront checkout | One-time hosted Airwallex payment link, canonical order, signed webhook, idempotent paid transition | Payment-link and payment-intent webhook events | Storefront publication + provider readiness | Requires configured linked payment account/webhook secret and sandbox/live acceptance; browser redirects never prove payment |
| Payouts | EXISTS | Finance Payouts workspace | `NUMERIC` paid-revenue balance, beneficiary references, idempotent request/submit, provider status webhooks | Payout transfer events and reconciliation | `payouts.request/manage` | Beneficiary onboarding, account funding/settlement and provider live acceptance remain operational launch gates |
