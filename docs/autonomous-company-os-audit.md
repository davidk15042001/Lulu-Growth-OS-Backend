...
| Analytics/metrics | PARTIAL | Consolidated analytics/statistics views | Google Analytics now discovers tenant-accessible GA4 properties through the Admin API and reads a bounded 28-day Data API report during sync; canonical metrics plus many generic report resources remain | Metric events | Read/reporting capabilities | Several generated dashboards, attribution models and write-side analytics workflows still need dedicated provider coverage |
