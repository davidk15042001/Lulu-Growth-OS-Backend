...
- Office voice now has workspace-scoped sessions and timestamped transcript evidence, governed spoken confirmations for critical actions, server-side TTS/realtime usage metering hooks, local VAD interruption handling, browser fallback, retry-safe client session/event IDs, bounded transcript reads, and user-controlled session deletion. The deployed server currently reports no `OPENAI_API_KEY`, so the browser fallback is active; WebRTC Realtime and server TTS become available after the server-side key and current provider rates are configured. Physical mobile/browser acceptance tests and a full-duplex audio output path remain deployment/provider gates.
- The authenticated assistant gateway now exposes the full registered canonical action surface: CRM enrichment/sync and pipeline transitions, quotes, calendar events, commerce orders/products/categories/inventory/fulfillment, website content/domain operations, product-image jobs, invoice lifecycle, payout requests/submission, messaging, email drafts/sending, reviews, social publishing/retry/cancel, and advertising. Financial and external-delivery actions require an exact confirmation package; prepared actions can be cancelled before an external side effect; executing actions fail closed into reconciliation. Users can export a bounded conversation package containing messages, action receipts, and voice evidence.
- `npm run eval:ai` is now part of the backend release gate. It verifies that every registered agent command has a server capability, unknown actions fail closed, and advertising cannot bypass the customer-funded budget boundary.
- AI funding and paid-media funding are independent runtime boundaries: a workspace with AI tokens but no ad-spend balance keeps general, CRM, finance, website, marketing and communication automation active while the paid advertising module and campaign execution remain paused.
...
- **Still requiring a provider or operations window:** real UnifyPort send/receive/webhook acceptance, real Airwallex storefront payment and payout acceptance, the existing payment-to-invoice acceptance, live mutation tests for external marketing/CMS/CRM providers, and backup/restore plus incident-recovery drills.
...
| Multi-agent collaboration | EXISTS | Office contributors, involved agents and per-run conversation | Parent plan delegates to persisted selected specialists; planning, evidence, handoffs, proposals, verification and terminal outcomes are recorded in an idempotent tenant-scoped collaboration ledger and handed to the next step as bounded untrusted evidence | Run steps, collaboration messages and action packets | Agent permissions + command policy | Zep mirrors the same run thread for long-term recall; conflict resolution remains deterministic policy/reviewer based, not unrestricted agent debate |
| Registered execution commands | PARTIAL | Workspace action outcomes | 37 server-owned command types with risk, budget and idempotency policy; CRM company enrichment/sync, CRM/Sales pipeline transitions, autonomous quote creation/delivery, guarded invoice issue/delivery, native calendar event creation, website generation/domain verification/publish, and canonical product/category create/update execute through domain services | Command packet lifecycle | Canonical service permissions | The remaining generated Workspace pages use an explicit `record.create_artifact` planning boundary; they no longer fail or claim an external side effect, but still need a dedicated domain adapter before autonomous mutation is enabled |
...
| Storefront customer payments | EXISTS | Published storefront checkout | One-time hosted Airwallex payment link, canonical order, signed webhook, idempotent paid transition, and tenant-scoped refund/dispute adjustment ledger | Payment-link, payment-intent, refund and dispute webhook events | Storefront publication + provider readiness | Requires configured linked payment account/webhook secret and sandbox/live acceptance; browser redirects never prove payment; customer refunds and disputes now reduce eligible proceeds, while settlement acceptance remains an operational gate |
| Payouts | EXISTS | Finance Payouts workspace | `NUMERIC` paid-revenue balance, capped net proceeds after active refunds/disputes, beneficiary references, idempotent request/submit, provider status webhooks | Payout transfer events and reconciliation | `payouts.request/manage` | Beneficiary onboarding, account funding/settlement, fees/tax treatment, dual-control policy and provider live acceptance remain operational launch gates |
...
| Website template selection | EXISTS | Website workspace | Verified product-aware template resolver and persisted generation plan | One-product storefront rendering, standard multi-page rendering, and explicit/automatic template choice | `website.read/manage` | Explicit one-product selection requires exactly one verified active or draft product; automatic selection falls back to the standard template when services or multiple offerings are present |
...
| Executive operating system | EXISTS | Virtual Office daily company position, risks, transparent forecasts, scenarios, plan approvals and verified plan outcomes | Tenant-scoped schedules, leased daily/weekly cycles, cutoff-bounded facts, indexed canonical CRM/Sales follow-up and opportunity aging risks, product/campaign metric-decline findings, explicit data gaps, two-point trend forecasts, numeric scenarios, deterministic Company Brain specialist teams, append-only proposal/learning evidence and calibration | Executive worker and Company Brain plan-only hand-off | `agents.read/manage`, proposal domain capability, and `quality.review` for outcome verification | External marketing, CRM, finance, product and provider changes remain on their canonical services and require their existing funding, permission, provider, settlement and compliance evidence; an approved executive proposal never directly performs an external side effect |
...
- Each Executive cycle stores a workspace-scoped data cutoff and data gaps; timestamped fact reads must apply that cutoff, forecasts must name their model and later calibration evidence, while scenarios cannot mutate canonical metrics.
- Executive proposal approval must recheck the proposal's domain capability, use its deterministic plan-only Company Brain team, and keep all direct provider/financial/customer mutations on canonical domain services. CRM age findings must be drawn only from tenant-scoped canonical records and redacted without `crm.read`. A plan outcome requires completed mission evidence, `quality.review`, optimistic versioning, and a replay-safe learning record.
