...
- The authenticated assistant gateway now exposes canonical actions for quotes, calendar events, commerce orders/products, website content jobs, product-image jobs, invoices, messaging, email drafts, reviews, and advertising. Prepared actions can be cancelled before an external side effect; executing actions fail closed into reconciliation. Users can export a bounded conversation package containing messages, action receipts, and voice evidence.
