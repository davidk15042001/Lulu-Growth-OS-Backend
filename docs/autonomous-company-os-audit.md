...
| Analytics/metrics | PARTIAL | Consolidated analytics/statistics views | Google Analytics now has a real tenant-scoped OAuth identity/readiness/health/discovery/sync adapter; canonical metrics plus many generic report resources remain | Metric events | Read/write capabilities | Property-level reporting query models and several generated dashboards still need dedicated provider coverage |
