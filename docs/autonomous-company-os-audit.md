...
| CRM companies | PARTIAL | Canonical companies-only CRM workspace | Workspace records plus company-intelligence enrichment | Record events, research worker, autonomous `crm.company.enrich` command | `crm.read/manage` | Company schema/enrichment is useful but still coexists with generic record storage |
...
| Website generation | EXISTS | Website workspace | Persisted generation jobs and worker | Website generation events, autonomous domain verification and publish commands | `website.read/manage` | Provider-specific publish support varies |
