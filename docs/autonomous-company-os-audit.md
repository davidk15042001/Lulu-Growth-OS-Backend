...
| Agent registry and team selection | EXISTS | Agent activity surfaces | 145 capability contracts and performance-based selection | Scheduled/reactive run events | `agents.read/execute/manage` | Definitions are not 145 permanent processes, by design |
...
| Registered execution commands | PARTIAL | Workspace action outcomes | Server-owned allowlist, risk and budget policy; concrete command materialization | Command packet lifecycle | Canonical service permissions | CRM/Sales pipeline transitions, autonomous quote creation/delivery, and guarded invoice issue/delivery now execute through canonical services; command vocabulary still does not cover every generated Workspace page |
| Autonomous scheduler | EXISTS | Observed status only | Automatic cycles with durable per-workspace cadence (15 minutes–24 hours), dedupe, and funded-workspace checks | Durable scheduled events | Entitlement + AI funds | UI for cadence editing should be surfaced in a dedicated settings surface; provider-specific schedules remain integration-dependent |
| Reactive autonomy | EXISTS | Timeline reflects resulting work | Source-event identity is persisted on idempotent employee runs; record, integration, funding, message, commerce, finance, media, website and social events are routed to responsible employees | Durable event consumers with exact event dedupe | Same policies as manual paths | Agent-originated outputs are excluded from reactive re-entry; new domains extend the explicit event map |
...
| Office projection | EXISTS | CEO KPIs, departments, employees, timeline, work panel | Digital employees, capabilities, work items, assignments, dependencies, attempts, events | Projects real runs/steps/action packets | `agents.read/manage` | Unavailable capabilities remain visible but never appear falsely active |
...
| CRM companies | PARTIAL | Canonical companies-only CRM workspace | Workspace records plus company-intelligence enrichment | Record events, research worker | `crm.read/manage` | Company schema/enrichment is useful but still coexists with generic record storage |
...
| Company intelligence | PARTIAL | Company fields and generated descriptions | Research/enrichment service and worker | Company record events | CRM permissions | Internet coverage and result quality depend on DataForSEO/provider configuration |
| Sales leads/opportunities/tasks | PARTIAL | Consolidated Workspace routes plus `/sales-pipeline` state API | Tenant-scoped `workspace_records`; canonical quotes are separate | Durable record events, follow-up commands, and guarded pipeline transitions | Leads/opportunities/quotes capabilities | Lead/opportunity/task state machines now enforce allowed transitions and optimistic locking; provider CRM sync and full UI adoption remain |
...
| Website generation | EXISTS | Website workspace | Persisted generation jobs and worker | Website generation events | `website.read/manage` | Provider-specific publish support varies |
| WordPress publishing | PARTIAL | Website/CMS pages | Real REST integration and publish verification | Website job events | `website.publish` | Capability depends on endpoint credentials and WordPress configuration |
| Webflow CMS | PARTIAL | Website/CMS pages | Real API paths for discovered sites/collections | Website jobs | `website.publish` | Not every CMS operation has end-to-end tests against a live tenant |
| Shopify | PARTIAL | Connection setup and commerce pages | OAuth metadata/provider catalog; product/order capability declarations | Provider events | Provider/commerce permissions | Adapter verification and canonical sync are incomplete; UI must not claim a sync without provider evidence |
...
| OmniChannel | EXISTS | Canonical conversations, manual send/note/takeover | Conversations/messages/routing, SSE, provider identities | Message lifecycle + AI reply worker | `omnichannel.read/reply/manage` | Channel availability must follow provider capability state |
...
| Facebook/Instagram Messenger | PARTIAL | Integration catalog | Provider records exist; end-to-end adapters are incomplete | Provider events only | Provider permissions | Do not present as live until webhook, identity, send and delivery paths pass provider E2E |
| Social publishing | PARTIAL | Canonical accounts, content and publication jobs in Marketing | Tenant-scoped content/jobs/attempts, optimistic locking, retries and dead-letter handling | Social lifecycle events and autonomous publish/retry/cancel commands | Workspace editor + provider scopes | Real Facebook Page and Instagram Business single-image publishing exists; live Meta review, credentials and reauthorization remain external launch gates |
| Google Ads | PARTIAL | Budget/optimization pages | Hard authorization and full-cap reservation, immutable payer mapping, real campaign mutation, observed-cost settlement and final invoice-coverage release | Ad events/action packets plus critical reconciliation worker | Advertising capabilities | Backend is fail-closed to verified CNY custom-period campaigns; live Google approval, credentials, managed-payer setup and sandbox/production evidence remain external gates |
| Meta/TikTok/LinkedIn Ads | PARTIAL | Integration and analytics pages | Catalog/generic records | No complete mutation adapter | Advertising capabilities | Analysis may be shown only from imported evidence; autonomous spend mutations remain disabled |
...
| Analytics/metrics | PARTIAL | Consolidated analytics/statistics views | Canonical metrics plus many generic report resources | Metric events | Read/write capabilities | Several generated dashboards still have no dedicated provider query model |
