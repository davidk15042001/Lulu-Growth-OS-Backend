...
| Executive operating system | EXISTS | Virtual Office daily company position, risks, transparent forecasts, scenarios and plan approvals | Tenant-scoped schedules, leased daily/weekly cycles, fixed data cutoffs, findings, explicit data gaps, two-point trend forecasts, numeric scenarios, append-only proposal/learning evidence and calibration | Executive worker and Company Brain plan-only hand-off | `agents.read/manage` plus the proposal domain capability | External marketing, CRM, finance, product and provider changes remain on their canonical services and require their existing funding, permission, provider, settlement and compliance evidence; an approved executive proposal never directly performs an external side effect |
...
- Each Executive cycle stores a workspace-scoped data cutoff and data gaps; forecasts must name their model and later calibration evidence, while scenarios cannot mutate canonical metrics.
- Executive proposal approval must recheck the proposal's domain capability and keep all direct provider/financial/customer mutations on canonical domain services.
