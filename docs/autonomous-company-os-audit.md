...
| CRM companies | PARTIAL | Canonical companies-only CRM workspace | Workspace records plus company-intelligence enrichment and immutable per-attempt research snapshots; Salesforce, HubSpot and Pipedrive now have tenant-scoped read-only company probes in the Provider Control Plane | Record events, research worker, autonomous `crm.company.enrich` command | `crm.read/manage` | Provider write sync and a dedicated company schema remain future work |
