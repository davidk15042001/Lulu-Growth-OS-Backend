...
function fakeMetaClient(provider: 'FACEBOOK' | 'INSTAGRAM', publish?: () => Promise<never>) {
  return {
    ...
    async readAdSpend() {
      return { spend: '12.34', impressions: '100', clicks: '4', dateStart: '2026-01-01', dateStop: '2026-01-31', providerRequestId: 'ads-insights-trace' };
    },
...
describe('Meta Graph publishing client', () => {
  it('uses bearer credentials and the real Facebook and Instagram publishing endpoints', async () => {
    ...
    const fetchImpl = (async (request: URL | RequestInfo, init?: RequestInit) => {
      ...
      if (url.pathname.endsWith('/act_12345/insights')) return Response.json({ data: [{ spend: '12.34', impressions: '100', clicks: '4', date_start: '2026-01-01', date_stop: '2026-01-31' }] }, { headers: { 'x-fb-trace-id': 'trace-insights' } });
    ...
    const spend = await client.readAdSpend({ accessToken: 'user-secret', adAccountId: adAccount.adAccountId });
    ...
    assert.equal(spend.spend, '12.34');
    assert.deepEqual(calls.map((call) => call.url.pathname), ['/v23.0/12345','/v23.0/act_12345','/v23.0/act_12345/campaigns','/v23.0/act_12345/insights','/v23.0/12345/feed','/v23.0/98765/media','/v23.0/container-1','/v23.0/98765/media_publish']);
    ...
    assert.match(calls[4]?.body ?? '', /link=https%3A%2F%2Fexample.com%2Fproduct/);
  });
