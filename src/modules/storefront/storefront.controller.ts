...
const STOREFRONT_UI: Record<StorefrontLocale, { language: string; services: string; serviceFallback: string; shop: string; products: string; noProducts: string; request: string; requestBody: string; noSelection: string; email: string; note: string; notePlaceholder: string; send: string; poweredBy: string; welcome: string; introduction: string; add: string; quote: string; added: string; requestReceived: string; selectFirst: string; failed: string; productFallback: string }> = {
  de: { language: 'Sprache', services: 'Leistungen', serviceFallback: 'Leistungen und Lösungen', shop: 'Online-Shop', products: 'Produkte', noProducts: 'Noch keine öffentlich freigegebenen Produkte.', request: 'Produktanfrage senden', requestBody: 'Wähle Produkte aus und sende dem Unternehmen eine Anfrage. Es wird keine Online-Zahlung durchgeführt.', noSelection: 'Noch keine Produkte ausgewählt.', email: 'E-Mail', note: 'Nachricht / Lieferinformationen', notePlaceholder: 'Wie können wir dir helfen?', send: 'Anfrage senden', poweredBy: 'Powered by Lulu AI', welcome: 'Willkommen bei', introduction: 'Entdecke unsere Produkte und Leistungen.', add: 'Anfrage hinzufügen', quote: 'Dieses Produkt benötigt zuerst ein Angebot.', added: 'Produkt zur Anfrage hinzugefügt.', requestReceived: 'Anfrage erhalten.', selectFirst: 'Bitte wähle zuerst mindestens ein Produkt aus.', failed: 'Die Anfrage konnte nicht verarbeitet werden.', productFallback: 'Kein Bild' },
  en: { language: 'Language', services: 'Services', serviceFallback: 'Services and solutions', shop: 'Online shop', products: 'Products', noProducts: 'No publicly available products yet.', request: 'Send a product request', requestBody: 'Select products and send the company a request. No online payment is processed.', noSelection: 'No products selected yet.', email: 'Email', note: 'Message / delivery information', notePlaceholder: 'How can we help?', send: 'Send request', poweredBy: 'Powered by Lulu AI', welcome: 'Welcome to', introduction: 'Discover our products and services.', add: 'Add to request', quote: 'This product requires a quote first.', added: 'Product added to your request.', requestReceived: 'Request received.', selectFirst: 'Please select at least one product first.', failed: 'The request could not be processed.', productFallback: 'No image' },
  zh: { language: '语言', services: '服务', serviceFallback: '服务与解决方案', shop: '在线商店', products: '产品', noProducts: '暂无公开产品。', request: '发送产品咨询', requestBody: '选择产品并向企业发送咨询。此处不进行在线支付。', noSelection: '尚未选择产品。', email: '电子邮件', note: '留言 / 配送信息', notePlaceholder: '我们可以如何帮助你？', send: '发送咨询', poweredBy: '由 Lulu AI 提供支持', welcome: '欢迎来到', introduction: '探索我们的产品和服务。', add: '加入咨询', quote: '此产品需要先获取报价。', added: '产品已加入咨询。', requestReceived: '已收到咨询。', selectFirst: '请先选择至少一个产品。', failed: '无法处理此请求。', productFallback: '暂无图片' }
};
...
export async function render(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const productCards = value.products.map((item) => item.imageUrl
      ? `<article class="product"><img src="${escapeHtml(safeImageUrl(item.imageUrl))}" alt="${escapeHtml(item.imageAlt || item.name)}"><div class="product-body"><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml(item.shortDescription || item.longDescription || '')}</p><strong>${escapeHtml(item.price ? `${item.price} ${item.currency || ''}` : ui.products)}</strong><button class="add" type="button" data-product-id="${escapeHtml(item.id)}">${escapeHtml(ui.add)}</button></div></article>`
      : `<article class="product"><div class="product-placeholder"><span>${escapeHtml(ui.productFallback ?? ui.products)}</span></div><div class="product-body"><h3>${escapeHtml(item.name)}</h3><p>${escapeHtml(item.shortDescription || item.longDescription || '')}</p><strong>${escapeHtml(item.price ? `${item.price} ${item.currency || ''}` : ui.products)}</strong><button class="add" type="button" data-product-id="${escapeHtml(item.id)}">${escapeHtml(ui.add)}</button></div></article>`).join('');
