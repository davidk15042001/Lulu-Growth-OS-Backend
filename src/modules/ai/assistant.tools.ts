...
export function buildAssistantTools(conversationId: string): AssistantTool[] {
  return [
    ...
    },
    {
      ...
      description:
        'Execute a real workspace action through the canonical workspace service. Use this only when the user explicitly asks to create, send, publish, update, or otherwise operate something. Financial transfers, invoice issuing/sending, external messages, email delivery, social publishing, website publishing, and payout submission require an exact confirmation package before execution. For invoices, first use list_records to resolve the exact customer when possible, then request finance.invoice.create_and_send. Paid advertising requires both funded ad spend and an active customer authorization scoped to the exact provider account, campaign, currency, period and amount.',
      parameters: {
        type: 'object',
        properties: {
          type: {
