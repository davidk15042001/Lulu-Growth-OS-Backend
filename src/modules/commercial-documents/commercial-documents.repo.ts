...
export async function getQuote(workspaceId: string, id: string) {
  ...
  return { quote: quote.rows[0], sellerProfile: await getCurrentDocumentSellerProfile(workspaceId), versions: versions.rows, lines: lines.rows, deliveries: deliveries.rows };
}
...
export async function sendQuote(workspaceId: string, quoteId: string, actorId: string, input: SendDocumentInput) {
  return withTransaction(async (client) => {
    ...
    if (!['DRAFT','READY','SENT'].includes(row.version_status)) throw conflictError('Only a ready quote can be sent');
...
export async function getInvoice(workspaceId:string,id:string,client?:PoolClient){
  ...
  return {invoice:invoice.rows[0],sellerProfile:isDocumentSellerProfile(snapshot)?snapshot:currentSellerProfile,buyerProfile:currentSellerProfile,lines:lines.rows,deliveries:deliveries.rows,payments};
}
...
export async function issueInvoice(workspaceId:string,id:string,actorId:string,sellerProfileOverride?:DocumentSellerProfile){return withTransaction(async(client)=>{const row=(await query<any>(`SELECT * FROM invoices WHERE workspace_id=$1 AND id=$2 FOR UPDATE`,[workspaceId,id],client)).rows[0];if(!row)throw notFoundError('Invoice not found');if(!['DRAFT','READY'].includes(row.status))throw conflictError('Only a ready invoice can be issued');const lines=await query(`SELECT 1 FROM invoice_lines WHERE workspace_id=$1 AND invoice_id=$2 LIMIT 1`,[workspaceId,id],client);if(!lines.rows[0])throw conflictError('Invoice requires at least one line item');const sellerProfile=sellerProfileOverride??await getCurrentDocumentSellerProfile(workspaceId,client);if(!sellerProfile)throw conflictError('A company profile is required before issuing an invoice');const sellerType=sellerProfileOverride?'LULU_PLATFORM':'WORKSPACE';const token=await createLink(client,workspaceId,'INVOICE',id,null);const documentReference=`/documents/commercial/${token}`;const documentHash=hash(JSON.stringify({invoiceId:id,total:row.grand_total,version:row.version}));await query(`UPDATE invoices SET status='ISSUED',issue_date=COALESCE(issue_date,CURRENT_DATE),issued_at=NOW(),amount_due=grand_total,document_status='READY',document_storage_reference=$6,document_hash=$7,metadata=jsonb_set(jsonb_set(COALESCE(metadata,'{}'::jsonb),'{sellerProfile}',$4::jsonb,true),'{sellerType}',$5::jsonb,true),updated_by=$3 WHERE workspace_id=$1 AND id=$2`,[workspaceId,id,actorId,JSON.stringify(sellerProfile),JSON.stringify(sellerType),documentReference,documentHash],client);await audit(client,workspaceId,actorId,'invoice.issued','invoice',id,{status:'ISSUED',sellerProfileCaptured:true,sellerType,documentStatus:'READY'});await appendDomainEvent({workspaceId,type:DOMAIN_EVENT_TYPES.INVOICE_ISSUED,aggregateType:'invoice',aggregateId:id,payload:{invoiceId:id},metadata:{actorId,source:'commercial-documents'},idempotencyKey:`invoice:${id}:issued`},client);return getInvoice(workspaceId,id,client);});}

/**
 * Make the canonical PDF link available without sending the invoice.
 *
 * Issuing an invoice is enough to make it legally inspectable, including for
 * automatic prepaid-balance invoices.  Historically only `sendInvoice`
 * created a public document link, leaving automatically paid invoices with a
 * PENDING document status even though the authenticated PDF endpoint worked.
 * This helper repairs that state idempotently and is safe to call from both
 * the live payment path and the historical reconciliation worker.
 */
export async function ensureInvoiceDocument(workspaceId: string, invoiceId: string, actorId: string | null = null) {
  return withTransaction(async (client) => {
    const row = (await query<{ id: string; status: string; grandTotal: string; version: number; documentStatus: string; documentStorageReference: string | null; documentHash: string | null }>(
      `SELECT id,status,grand_total::text AS "grandTotal",version,
              document_status AS "documentStatus",
              document_storage_reference AS "documentStorageReference",
    ...
    if (!row) return null;
    if (row.documentStorageReference && row.documentStatus === 'READY') return row.documentStorageReference;
    const token = await createLink(client, workspaceId, 'INVOICE', invoiceId, null);
    const reference = `/documents/commercial/${token}`;
    const documentHash = hash(JSON.stringify({ invoiceId, total: row.grandTotal, version: row.version }));
    await query(
      `UPDATE invoices
          SET document_status='READY',document_storage_reference=$3,document_hash=$4,updated_by=COALESCE($5,updated_by),updated_at=NOW()
        WHERE workspace_id=$1 AND id=$2`,
      [workspaceId, invoiceId, reference, documentHash, actorId],
      client,
    );
    return reference;
  });
