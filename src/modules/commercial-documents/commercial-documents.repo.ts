...
export async function getInvoice(workspaceId:string,id:string){const invoice=await query(`SELECT ${invoiceSelect} FROM invoices i WHERE i.workspace_id=$1 AND i.id=$2`,[workspaceId,id]);if(!invoice.rows[0])return null;const lines=await query(`SELECT id,product_id AS "productId",sku_snapshot AS "sku",product_name_snapshot AS "productName",description_snapshot AS "description",quantity,quantity_unit AS "quantityUnit",unit_price AS "unitPrice",discount,tax,line_total AS "lineTotal",sort_order AS "sortOrder" FROM invoice_lines WHERE workspace_id=$1 AND invoice_id=$2 ORDER BY sort_order,id`,[workspaceId,id]);const deliveries=await query(`SELECT id,document_type AS "documentType",document_id AS "documentId",conversation_id AS "conversationId",channel,recipient,status,provider_message_id AS "providerMessageId",failure_reason AS "failureReason",actor_type AS "actorType",actor_id AS "actorId",sent_at AS "sentAt",delivered_at AS "deliveredAt",failed_at AS "failedAt",created_at AS "createdAt" FROM document_deliveries WHERE workspace_id=$1 AND document_type='INVOICE' AND document_id=$2 ORDER BY created_at DESC`,[workspaceId,id]);return {invoice:invoice.rows[0],lines:lines.rows,deliveries:deliveries.rows};}
...
export async function getPublicDocument(token:string){const result=await query<any>(`SELECT l.workspace_id,l.document_type,l.document_id,q.quote_number,q.status AS quote_status,q.currency AS quote_currency,q.language AS quote_language,qv.version_number,qv.subtotal,qv.discount_total,qv.shipping_total,qv.tax_total,qv.grand_total,qv.valid_until,qv.terms_snapshot, i.invoice_number,i.status AS invoice_status,i.currency AS invoice_currency,i.language AS invoice_language,i.issue_date,i.due_date,i.subtotal AS invoice_subtotal,i.discount_total AS invoice_discount_total,i.shipping_total AS invoice_shipping_total,i.tax_total AS invoice_tax_total,i.grand_total AS invoice_grand_total,i.amount_paid,i.amount_due FROM commercial_document_links l LEFT JOIN quotes q ON l.document_type='QUOTE' AND q.workspace_id=l.workspace_id AND q.id=l.document_id LEFT JOIN quote_versions qv ON qv.workspace_id=q.workspace_id AND qv.id=q.current_version_id LEFT JOIN invoices i ON l.document_type='INVOICE' AND i.workspace_id=l.workspace_id AND i.id=l.document_id WHERE l.token_hash=$1 AND l.revoked_at IS NULL AND (l.expires_at IS NULL OR l.expires_at>NOW())`,[hash(token)]);const row=result.rows[0];if(!row)return null;if(row.document_type==='QUOTE'){const lines=await query(`SELECT product_name_snapshot AS "productName",description_snapshot AS "description",quantity,quantity_unit AS "quantityUnit",unit_price AS "unitPrice",discount,tax,line_total AS "lineTotal" FROM quote_lines WHERE workspace_id=$1 AND quote_version_id=(SELECT current_version_id FROM quotes WHERE workspace_id=$1 AND id=$2) ORDER BY sort_order`,[row.workspace_id,row.document_id]);return {type:'QUOTE',number:row.quote_number,status:row.quote_status,currency:row.quote_currency,language:row.quote_language,versionNumber:row.version_number,validUntil:row.valid_until,subtotal:row.subtotal,discountTotal:row.discount_total,shippingTotal:row.shipping_total,taxTotal:row.tax_total,grandTotal:row.grand_total,terms:row.terms_snapshot,lines:lines.rows};}const lines=await query(`SELECT product_name_snapshot AS "productName",description_snapshot AS "description",quantity,quantity_unit AS "quantityUnit",unit_price AS "unitPrice",discount,tax,line_total AS "lineTotal" FROM invoice_lines WHERE workspace_id=$1 AND invoice_id=$2 ORDER BY sort_order`,[row.workspace_id,row.document_id]);return {type:'INVOICE',number:row.invoice_number,status:row.invoice_status,currency:row.invoice_currency,language:row.invoice_language,issueDate:row.issue_date,dueDate:row.due_date,subtotal:row.invoice_subtotal,discountTotal:row.invoice_discount_total,shippingTotal:row.invoice_shipping_total,taxTotal:row.invoice_tax_total,grandTotal:row.invoice_grand_total,amountPaid:row.amount_paid,amountDue:row.amount_due,lines:lines.rows};}
