...
export function renderOneProductLanding(profile: WebsiteContentProfile, palette: ThemePalette, images: WebsiteImageAsset[]): RenderedPage {
  ...
  const volt = '#d4ff3d';
  const mist = '#9db4c4';
  const glass = 'rgba(255,255,255,.05)';
  const glassLine = 'rgba(255,255,255,.10)';
  const panel = '#0d141b';
  ...
  const productImage = imageMarkup(images[0], 520) || `<div role="img" aria-label="${escapeHtml(labels.fallback)}" style="display:flex;min-height:520px;align-items:center;justify-content:center;border:1px dashed ${glassLine};border-radius:40px;background:${glass};color:${mist};font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.16em;text-transform:uppercase"><div style="display:flex;flex-direction:column;align-items:center;gap:8px"><span style="color:${volt};font-size:32px">+</span><span>${escapeHtml(labels.fallback)}</span></div></div>`;
  const detailImage = imageMarkup(images[1], 420) || `<div role="img" aria-label="${escapeHtml(labels.fallback)}" style="display:flex;min-height:420px;align-items:center;justify-content:center;border:1px dashed ${glassLine};border-radius:28px;background:${glass};color:${mist};font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.16em;text-transform:uppercase"><span style="color:${volt}">${escapeHtml(labels.fallback)}</span></div>`;
  const benefitCards = benefits.map((card, index) => `<article data-lulu-card="product-benefit" style="height:100%;border:1px solid ${glassLine};border-radius:28px;background:${glass};padding:32px;color:${frost}"><span style="font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:12px;color:${volt}">${['(a)', '(b)', '01'][index]}</span><h3 style="margin:8px 0 0;color:${frost};font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:30px;font-weight:700;line-height:1.1;text-transform:uppercase">${escapeHtml(card.title)}</h3><p style="max-width:40ch;margin:12px 0 0;color:${mist};line-height:1.7">${escapeHtml(card.description)}</p></article>`).join('');
  const faqItems = faqs.map((faq) => `<details data-lulu-product-faq style="padding:17px 0;border-bottom:1px solid ${glassLine}"><summary style="cursor:pointer;color:${frost};font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:20px;text-transform:uppercase">${escapeHtml(faq.question)}<span style="float:right;color:${mist}">+</span></summary><p style="margin:10px 0 0;color:${mist};line-height:1.7">${escapeHtml(faq.answer)}</p></details>`).join('');
  const darkPalette: ThemePalette = { ...palette, primary: volt, secondary: panel, accent: volt, ink: frost, muted: mist, surface: glass, background: ink };
  return {
    openingHtml: templatePageOpening(darkPalette, ONE_PRODUCT_TEMPLATE_KEY),
    sections: [
      renderedSection('product-hero', profile.home.eyebrow, `${TEMPLATE_FONT_LINKS}<section data-lulu-section="product-hero" style="width:100%;margin:0;padding:clamp(72px,10vw,120px) 16px;background:${ink};color:${frost}"><div style="position:relative;display:grid;max-width:1280px;margin:0 auto;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));align-items:center;gap:48px"><div><p style="margin:0;color:${volt};font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.2em;text-transform:uppercase">${escapeHtml(profile.home.eyebrow)}</p><h1 style="max-width:760px;margin:16px 0 0;color:${frost};font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:clamp(56px,9vw,112px);font-weight:700;line-height:.9;text-transform:uppercase">${escapeHtml(profile.home.headline || product.name)}</h1><p style="max-width:650px;margin:24px 0 0;color:${mist};font-size:18px;line-height:1.7">${escapeHtml(profile.home.introduction || product.description)}</p><div style="display:flex;flex-wrap:wrap;gap:12px;margin-top:32px">${button(labels.cta, '#product-request', darkPalette)}<a href="#product-benefits" style="display:inline-flex;min-height:46px;align-items:center;justify-content:center;padding:12px 18px;border:1px solid ${glassLine};border-radius:999px;color:${frost};font-size:15px;text-decoration:none">${escapeHtml(labels.details)} →</a></div></div><div style="position:relative"><div style="position:absolute;inset:0;transform:rotate(-6deg);border:1px solid ${glassLine};border-radius:40px;background:${glass}"></div><div style="position:absolute;inset:0;transform:rotate(3deg);border:1px solid ${glassLine};border-radius:40px;background:${glass}"></div><div style="position:relative">${productImage}</div></div></div></section>`),
      renderedSection('product-benefits', labels.benefits, `<section data-lulu-section="product-benefits" style="width:100%;margin:0;padding:64px 16px 80px;background:${ink}"><div style="max-width:1280px;margin:0 auto"><div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:16px">${benefitCards}</div></div></section>`),
      renderedSection('product-details', labels.details, `<section data-lulu-section="product-details" style="width:100%;margin:0;padding:64px 16px;background:${ink};border-top:1px solid ${glassLine}"><div style="display:grid;max-width:1280px;margin:0 auto;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));align-items:center;gap:40px"><div>${detailImage}</div><div style="font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:14px;color:${frost}"><div style="padding:14px 0;border-bottom:1px solid ${glassLine}"><span style="color:${mist}">${escapeHtml(labels.product)}</span><span style="float:right">${escapeHtml(product.name)}</span></div><div style="padding:14px 0;border-bottom:1px solid ${glassLine}"><span style="color:${mist}">${escapeHtml(labels.details)}</span><span style="float:right">${escapeHtml(product.category)}</span></div><div style="padding:14px 0;border-bottom:1px solid ${glassLine}"><span style="color:${mist}">Brand voice</span><span style="float:right">${escapeHtml(profile.brandVoice)}</span></div><div style="padding:14px 0"><span style="color:${mist}">Status</span><span style="float:right;color:${volt}">${escapeHtml(labels.proof)}</span></div></div></div></section>`),
      renderedSection('product-proof', labels.proof, `<section data-lulu-section="product-proof" style="width:100%;margin:0;padding:64px 16px;background:${ink};border-top:1px solid ${glassLine}"><div style="display:grid;max-width:1280px;margin:0 auto;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:16px">${trust.map((item, index) => `<blockquote data-lulu-product-trust style="height:100%;margin:0;border:1px solid ${glassLine};border-radius:24px;background:${glass};padding:28px"><p style="margin:0;color:${frost};font-size:18px">&quot;${escapeHtml(item)}&quot;</p><span style="display:block;margin-top:16px;color:${mist};font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px">- ${escapeHtml(labels.proof)} ${index + 1}</span></blockquote>`).join('')}</div></section>`),
      renderedSection('product-process', labels.process, `<section data-lulu-section="product-process" style="width:100%;margin:0;padding:72px 16px;background:${ink};border-top:1px solid ${glassLine};text-align:center"><div style="max-width:1280px;margin:0 auto"><span style="color:${frost};font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:clamp(56px,8vw,112px);text-transform:uppercase">${escapeHtml(product.name)}</span><p style="margin:12px 0 0;color:${mist};font-family:'IBM Plex Mono',ui-monospace,monospace">${escapeHtml(profile.home.finalCtaText || labels.requestBody)}</p><div style="display:flex;justify-content:center;margin-top:28px">${button(labels.cta, '#product-request', darkPalette)}</div><div style="display:none">${steps.map((step, index) => `<span data-lulu-card="product-step">${String(index + 1).padStart(2, '0')} ${escapeHtml(step.title)}</span>`).join('')}</div></div></section>`),
      renderedSection('product-faq', labels.faq, `<section data-lulu-section="product-faq" style="width:100%;margin:0;padding:64px 16px;background:${ink};border-top:1px solid ${glassLine}"><div style="max-width:800px;margin:0 auto"><div style="margin-top:20px">${faqItems}</div></div></section>`),
      renderedSection('product-request', labels.request, `<section data-lulu-section="product-request" id="product-request" style="width:100%;margin:0;padding:64px 16px 88px;background:${ink};border-top:1px solid ${glassLine};color:${frost}"><div style="max-width:800px;margin:0 auto"><h2 style="margin:0;color:${frost};font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:clamp(34px,5vw,52px);text-transform:uppercase">${escapeHtml(labels.request)}</h2><p style="margin:14px 0 0;color:${mist};line-height:1.7">${escapeHtml(labels.requestBody)}</p><div style="margin-top:26px;padding:24px;border:1px solid ${glassLine};border-radius:24px;background:${glass};color:${frost}">${contactFormPreviewMarkup(profile, darkPalette)}</div></div></section>`),
    ],
    closingHtml: '</main>',
  };
}
