...
import { markPlatformConnectionError } from '../onboarding/onboarding.repo.js';
...
async function providerRequest(provider: WebsiteProvider, url: string, token: string, init: RequestInit = {}): Promise<ProviderResponse> {
  if (provider !== 'managed') {
    throw new AppError(410, 'WEBSITE_PROVIDER_RETIRED', 'WordPress and Webflow integrations are retired; only Lulu-managed websites are supported.');
  }
...
async function tokenFor(_workspaceId: string, _provider: 'wordpress' | 'webflow'): Promise<never> {
  throw new AppError(410, 'WEBSITE_PROVIDER_RETIRED', 'WordPress and Webflow integrations are retired; only Lulu-managed websites are supported.');
}
