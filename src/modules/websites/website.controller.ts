...
export async function createJob(req: WorkspaceRequest, res: Response, next: NextFunction) { try { const params = siteIdParams.parse(req.params); const site = await repo.getSite(params.workspaceId, params.siteId); if (!site) throw new AppError(404, 'WEBSITE_SITE_NOT_FOUND', 'Website site was not found'); const job = await repo.createJob({ siteId: params.siteId, prompt: createJobSchema.parse(req.body).prompt, createdBy: req.user!.id }); if (!job) throw new AppError(500, 'WEBSITE_GENERATION_FAILED', 'Website generation job could not be created'); try { await repo.updateJob(params.siteId, job.id, { status: 'planning' }); const plan = await generateWebsitePlan({ workspaceId: params.workspaceId, userId: req.user!.id, prompt: job.prompt, provider: site.provider }); return createdResponse(res, 'Website generation preview created', await repo.updateJob(params.siteId, job.id, { status: 'preview', plan, preview: { provider: site.provider, ownershipMode: site.ownershipMode, pages: plan.pages.map((page) => ({ title: page.title, slug: page.slug, seoTitle: page.seoTitle, seoDescription: page.seoDescription })) } })); } catch (generationError) { await repo.updateJob(params.siteId, job.id, { status: 'failed', errorCode: generationError instanceof AppError ? generationError.code : 'WEBSITE_GENERATION_FAILED', errorMessage: generationError instanceof Error ? generationError.message : 'Website generation failed' }); throw generationError; } } catch (error) { next(error); } }
