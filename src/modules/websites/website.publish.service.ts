...
export function wordpressGutenbergContent(page: unknown) {
  ...
  return [
    ...
    '<div class="wp-block-group alignfull lulu-generated-page" style="padding:0;--wp--style--block-gap:0px">',
    blockSections,
    '</div>',
    '<!-- /wp:group -->',
  ].join('\n');
...
export function wordpressSiteChrome(plan: unknown, publishedPages: unknown[], siteUrl: string) {
  ...
  const language = String(contentProfile.primaryLanguage ?? 'en').trim().toLowerCase();
  const shortLabel = (slug: string, fallback: string) => {
    const german = /^(de|de[-_]|german|deutsch)/.test(language);
    const chinese = /^(zh|zh[-_]|chinese|中文|简体中文|繁體中文)/.test(language);
    if (slug === 'home') return german ? 'Startseite' : chinese ? '首页' : 'Home';
    if (slug === 'about') return german ? 'Über uns' : chinese ? '关于我们' : 'About';
    if (slug === 'services') return german ? 'Leistungen' : chinese ? '服务' : 'Services';
    if (slug === 'contact') return german ? 'Kontakt' : chinese ? '联系' : 'Contact';
    return fallback;
  };
  ...
  const header = htmlBlock(`${WORDPRESS_FONT_LINKS}<div data-lulu-global="header" data-lulu-design-source="custom-bolt-forge" style="width:100%;max-width:none;margin:0;font-family:Barlow,'Helvetica Neue',Arial,sans-serif"><div style="background:${escapeWordpressHtml(secondary)};color:#fff"><div style="display:flex;max-width:1280px;margin:0 auto;padding:8px 16px;align-items:center;justify-content:space-between;gap:8px 16px;flex-wrap:wrap"><span style="color:rgba(255,255,255,.8);font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.14em;text-transform:uppercase">${escapeWordpressHtml(tagline || siteTitle)}</span><a href="${escapeWordpressHtml(contactHref)}" style="color:${escapeWordpressHtml(accent)};font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px;font-weight:600;letter-spacing:.14em;text-decoration:none;text-transform:uppercase;white-space:nowrap">${escapeWordpressHtml(contactLabel)} →</a></div></div><div style="border-bottom:1px solid #dce2e8;background:#fff"><div data-lulu-header-row style="display:grid;min-height:72px;max-width:1280px;margin:0 auto;padding:12px 16px;grid-template-columns:repeat(auto-fit,minmax(min(100%,420px),1fr));align-items:center;gap:12px 24px"><a data-lulu-brand href="${escapeWordpressHtml(baseUrl)}" style="display:flex;min-width:0;align-items:center;gap:12px;color:${escapeWordpressHtml(ink)};text-decoration:none"><span aria-hidden="true" style="display:grid;width:36px;height:36px;flex:0 0 36px;place-items:center;background:${escapeWordpressHtml(primary)};color:#fff;font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:19px;font-weight:700">${escapeWordpressHtml(initial)}</span><span style="min-width:0"><strong style="display:block;font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:20px;line-height:1.05;overflow-wrap:anywhere">${escapeWordpressHtml(siteTitle)}</strong>${tagline ? `<small data-lulu-brand-caption style="display:block;margin-top:3px;color:${escapeWordpressHtml(muted)};font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:9px;letter-spacing:.08em;text-transform:uppercase;overflow-wrap:anywhere">${escapeWordpressHtml(tagline.slice(0, 72))}</small>` : ''}</span></a><div style="display:flex;min-width:0;align-items:center;justify-content:flex-end;gap:10px 16px;flex-wrap:wrap"><nav data-lulu-desktop-nav aria-label="Primary navigation" style="display:flex;align-items:center;justify-content:flex-end;gap:4px 14px;flex-wrap:wrap">${headerLinks}</nav><a data-lulu-header-cta href="${escapeWordpressHtml(contactHref)}" style="display:inline-flex;min-height:40px;align-items:center;justify-content:center;padding:10px 16px;background:${escapeWordpressHtml(primary)};border:1px solid ${escapeWordpressHtml(primary)};color:#fff;font-size:14px;font-weight:700;text-decoration:none;white-space:nowrap">${escapeWordpressHtml(contactLabel)}</a></div></div></div></div>`);
  const footer = htmlBlock(`<div data-lulu-global="footer" data-lulu-design-source="custom-bolt-forge" style="width:100%;max-width:none;margin:0;background:${escapeWordpressHtml(secondary)};color:#fff;font-family:Barlow,'Helvetica Neue',Arial,sans-serif"><div style="display:grid;max-width:1280px;margin:0 auto;padding:56px 16px;grid-template-columns:repeat(auto-fit,minmax(min(100%,190px),1fr));gap:40px"><div style="grid-column:span 2"><div style="display:flex;align-items:center;gap:12px"><span aria-hidden="true" style="display:grid;width:36px;height:36px;place-items:center;background:${escapeWordpressHtml(primary)};color:#fff;font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:19px;font-weight:700">${escapeWordpressHtml(initial)}</span><strong style="font-family:'Barlow Condensed','Arial Narrow',Arial,sans-serif;font-size:22px;overflow-wrap:anywhere">${escapeWordpressHtml(siteTitle)}</strong></div>${description ? `<p style="max-width:430px;margin:16px 0 0;color:#bdc8d0;font-size:14px;line-height:1.7">${escapeWordpressHtml(description)}</p>` : ''}</div><div><p style="margin:0 0 16px;color:#aeb9c1;font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.14em;text-transform:uppercase">Navigation</p><div role="navigation" aria-label="Footer navigation">${footerLinks}</div></div><div><p style="margin:0 0 16px;color:#aeb9c1;font-family:'IBM Plex Mono',ui-monospace,monospace;font-size:11px;letter-spacing:.14em;text-transform:uppercase">${escapeWordpressHtml(contactLabel)}</p><a href="${escapeWordpressHtml(contactHref)}" style="display:inline-flex;min-height:42px;align-items:center;justify-content:center;padding:11px 17px;background:${escapeWordpressHtml(primary)};border:1px solid ${escapeWordpressHtml(primary)};color:#fff;font-size:14px;font-weight:700;text-decoration:none">${escapeWordpressHtml(contactLabel)}</a></div></div><div style="max-width:1280px;margin:0 auto;padding:20px 16px;border-top:1px solid rgba(255,255,255,.16);color:#aeb9c1;font-size:13px">© ${new Date().getFullYear()} ${escapeWordpressHtml(siteTitle)}</div></div>`);
