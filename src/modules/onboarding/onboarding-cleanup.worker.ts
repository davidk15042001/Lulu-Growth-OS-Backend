...
const runtimeMonitor = createRuntimeWorkerMonitor('onboarding-cleanup', {
  staleAfterMs: Math.max(60_000, env.ONBOARDING_FILE_CLEANUP_INTERVAL_MINUTES * 60_000 + 60_000),
});
...
function requestOnboardingCleanupCycle(intervalMs: number): Promise<void> {
...
export function runOnboardingFileCleanupCycle(): Promise<void> {
  ...
  activeCycle = (async () => {
    let processed = 0;
    for (; processed < MAX_WORKSPACES_PER_CYCLE && !stopping; processed += 1) {
    ...
    runtimeMonitor.progress({ phase: 'processed', processed });
  })()
    .catch((error: unknown) => { runtimeMonitor.failed(error); logger.error({ error }, 'Onboarding file cleanup cycle failed'); })
...
export function startOnboardingFileCleanupWorker() {
  ...
  interval = setInterval(() => void requestOnboardingCleanupCycle(intervalMs), intervalMs);
  interval.unref();
  void requestOnboardingCleanupCycle(intervalMs);
  logger.info(
...
export async function stopOnboardingFileCleanupWorker() {
  stopping = true;
