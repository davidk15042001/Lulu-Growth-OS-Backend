...
const EnvSchema = z
  .object({
    ...
    OPENAI_VOICE_VAD_SILENCE_MS: z.coerce.number().int().min(150).max(2_000).default(500),
    OPENAI_VOICE_VAD_THRESHOLD: z.coerce.number().min(0).max(1).default(0.5),
    VOICE_REALTIME_COST_USD_PER_MINUTE: z.coerce.number().min(0).max(100).default(0),
    VOICE_TTS_COST_USD_PER_1K_CHARS: z.coerce.number().min(0).max(100).default(0),
    VOICE_CUSTOMER_MARKUP_MULTIPLIER: z.coerce.number().min(1).max(20).default(2),
